...
from .config import Config, MITM_HOST, INTERCEPT_PREFIXES
...
class MitmProxy:
    ...
    def serve(self) -> None:
        ...
        log.info("MITM proxy on http://127.0.0.1:%d (intercepts %s)",
                 self.cfg.proxy_port, MITM_HOST)
    ...
    def _intercept(self, tls: ssl.SSLSocket, method: str, path: str, body: bytes) -> None:
        ...
        if sub == "/bridge":
            return _send_json(tls, {
                "api_base_url": f"https://{MITM_HOST}", "expires_in": 14400,
                "worker_epoch": "1", "worker_jwt": "rcw-" + s.id})
        if sub == "/worker" and method == "GET":
            return _send_json(tls, {"worker": {
                "session_id": sid, "worker_epoch": str(s.worker_epoch),
        ...
        try:
            for ev in s.follow_downstream(self.stop.is_set):
                ...
                frame = ("event: client_event\nid: %d\ndata: %s\n\n"
                         % (ev.sequence_num, json.dumps(ev.wire()))).encode()
                tls.sendall(frame)
    ...
    # ---------------- passthrough / tunnel ----------------
    def _passthrough(self, tls: ssl.SSLSocket, method: str, path: str,
                     head: bytes, body: bytes) -> None:
        hdrs_text = head.decode("latin1").split("\r\n", 1)[1].rstrip("\r\n")
        # strip accept-encoding so we never have to gunzip; force close framing
        lines = hu.headers_without(hdrs_text, ("connection", "proxy-connection",
...
def _assistant_text(payload: dict[str, Any]) -> str:
    return "".join(b.get("text", "") for b in payload.get("message", {}).get("content", [])
