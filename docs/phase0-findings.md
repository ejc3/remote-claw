...
lower-fidelity tmux route. The current M1/M2/M3 evidence and later sequencing live in
...
boundaries. Codex M3b subsequently proved same-thread ChatGPT Remote text coexistence and
provider-transport isolation for exact 0.151.0/Linux arm64; the current release roadmap owns that live
evidence and its narrower limits. Keep tmux explicitly lower fidelity. Do not recreate the retired
generalized coordinator unless a concrete cross-adapter failure demonstrates that it is necessary.
