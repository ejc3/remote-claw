...
codecs, closed role/schema/bound/scope registry, and bounded raw digest helper. It parses no child
semantics and adds no artifact write, schema, repository, signer, pointer, port, runtime operation, or
production capability. E1b next owns every collector-backed Linux/OpenCode child codec and a real
collector fixture. E1c then retains exact signed OpenCode `user_text` native-binding evidence through
...
production operation, or capability claim. The implemented E0/E1a boundary and planned E1b/E1c/I design are
...
dispatch, or production wiring. A1.8a1-E1c/I first supplies only the dormant matched native-authority
...
- `packages/cli/src/host/state/{ids,native-binding-authority,native-binding-authority-evidence,path,validation,records,runtime,digests,protected,dispatch,backend}.ts`
  ...
  contracts and four deterministic builders and E1a's four strict ref-free parent codecs, commitment
  registry, and raw digest helper. The E0/E1a modules and tests are exported through the host-state
  barrel but contain no child semantic collector, authority operation, or runtime wiring.
...
- `packages/cli/src/host/state/{migration-v10,migration-v11,command-adjudication,command-result-finalization,command-adjudication-repository,command-adjudication-validator}.ts`,
  ...
  attempt/dispatch.
- No E1c/I migration, repository, signer boundary, validator, or operation exists yet. E1b's exact
  collector-backed native-binding child evidence remains planned, and A1.8a1-I's atomic
...
overlay, and inert `pending_seal` intent. A1.8a1-E1c/I first owns the inert evidence and
