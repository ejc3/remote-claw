...
loss left the local TUI usable. The separate 2026-09-08
...
| Codex recovery — complete for tested tuples | Same-thread fresh-projection restart and native work after broker loss | Packed CLI, local TUI and two browsers passed on Linux arm64 with 0.151.0/explicit WS/paginated and 0.153.4/managed Unix/paginated; [0.154.0 managed Unix/paginated](#codex-official-remote-recovery--complete) adds the actual Mac app and later work through reopened viewers |
...
This accepts the bounded text/image/interrupt/approval/question compatibility slice. The separate
[official desktop Remote recovery result](#codex-official-remote-recovery--complete) owns restart and
broker-loss qualification; this compatibility run itself does not requalify physical-phone/WebKit transport, native Mac
...
were unchanged. That 0.151.0 result did not claim managed-Unix/legacy recovery, a crash/restart of
app-server, or simultaneous official-Remote recovery.

**Managed run (2026-09-08).** The same sentinel passed on exact 0.153.4/Linux arm64 through
literal `unix://` with native-reported `historyMode:"paginated"`: packed CLI, built SQLite/viewer,
dedicated TUI, and phone/desktop browsers passed fresh-projection restart, unchanged native history,
duplicate-free backfill, and a fresh browser turn. After every broker-proxy connection closed, only the
companion exited with status 1 and a fresh TUI turn completed. Both screenshots were inspected;
the run took 12.2 seconds, 41 seconds including the build. Only the retained test changed to reuse
production client/URL/version/history helpers and record the observed tuple; no production recovery
or policy change was needed. The daemon, TUI, provider transport, authentication, and logged-in
browsers were preserved. Evidence is under
`/home/ubuntu/remote-claw-ui-artifacts/codex-managed-recovery-2026-09-08/`, outside Git. That run did not
qualify legacy recovery, simultaneous official Remote, native-process crash/restart, or stable
projection identity; the official client was not attached.

### Codex official Remote recovery — complete

**Bounded acceptance passed 2026-09-25:** exact Codex 0.154.0/Linux arm64, managed Unix socket,
native-reported paginated history, the existing TUI and actual Mac Codex desktop app on one already
attached EC2 thread through its existing desktop remote-host connection. The underlying desktop
connection transport was not separately qualified; this run did not exercise a provider-server/phone
Remote route or broaden M3b's historical provider-transport-isolation result.
A packed CLI from main `546c12e` and isolated SQLite/viewer broker supplied two
Chromium viewers at phone and desktop widths. Native daemon, TUI, account and permission policy were
not changed; all submitted markers requested exact text without tools or file changes.

Browser A appeared once in both viewers, native history, TUI and actual Mac app. Clean companion
restart created a different projection, backfilled A once and left native history unchanged; browser B
then completed once. Cutting only the test-owned broker proxy caused companion exit 1, while the
existing TUI and then the actual Mac app each completed a new native-origin turn on the same thread.
The Mac reply was confirmed visually and as one canonical native user/assistant pair while the proxy
remained down.

The original run stopped on a **helper selection race** during restoration: its lone-row selector
selected the previous projection without waiting for the new companion's connected presence. Cleanup
closed those viewer contexts. That failed run remains recorded; it is not reported as an end-to-end
pass. A separate tail reused the same broker database, identity, origin and native thread, waited for
a connected fresh projection, and reopened two viewers. Earlier prompts were not repeated. A/B/TUI/Mac
backfilled once, a single new final browser marker completed, and the actual Mac app and TUI showed
its reply. A read-only check confirmed all five user/assistant pairs once after reopening and reloading
on both layouts. Only test-owned processes were stopped afterward.

This closes companion restart/backfill, broker-loss isolation with the official **desktop** client,
and later browser work after selecting a fresh projection. It does not claim automatic same-tab or
stable-ID reconnect, native-daemon restart, legacy history, mobile-app Remote, physical-iPhone/WebKit
transport, pending approval/question recovery, or another native version/platform. Historical
0.151.0/0.153.4 results above retain their scope. No production or checked-in test/framework change was
needed; the retained sentinel was adapted only in private artifacts.

Evidence, including the failed original `acceptance.json`, passed `tail-acceptance.json`, exact
`reload-counts.json`, inspected viewer screenshots and root-inspected Mac screenshots, is under
`/home/ubuntu/remote-claw-ui-artifacts/codex-official-remote-recovery-2026-09-25.dluIvM/` outside Git.
Its `README.md` records the sequence and helper diagnosis.
