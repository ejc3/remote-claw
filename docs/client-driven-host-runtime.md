...
**Status:** selected architecture; A1.0 through dormant A1.8a0 are implemented. A1.8a1-E0 implements the six E-side canonical ID contracts and four deterministic attestation/snapshot ID derivations; A1.8a1-E1a now implements the four pure, strict, 64 KiB ref-free parent-envelope codecs, closed ordered role/schema/bound/scope registry, locked byte/digest vectors, and bounded raw-artifact digest helper. E1a parses content commitments only—not the committed Linux/OpenCode child semantics—and creates no artifact, SQLite schema or migration, repository, signer mutation, key-rotation barrier, transport-pointer write, callable-port access, owner operation, protocol packet, runtime wiring, or production capability. A1.8a1-E1b next owns every collector-backed child codec and a real collector fixture. A1.8a1-E1c then owns stateful accepted evidence under the frozen workspace, legacy-signer, phase-staging, and exact-reconciliation rules in the [technical reference](client-driven-host-runtime-reference.md#41-a18a1-native-binding-authority-freeze-planned-dormant), without changing either transport pointer. A1.8a1-I remains planned after E1c, A1.8a2 admitted atomic arming remains planned after I, and A1.8b still owns sealing and publishing.
...
**A1.8a1 native-binding authority (E0/E1a implemented; E1b/E1c/I planned):** E0 supplies identities. E1a supplies only the four canonical parents and ref-free `role/schema/digest/byteLength` commitments; every future `rcph_*` is repository-local graph state and never part of a parent, recipe, or caller request identity. E1b must implement the exact Linux/OpenCode child codecs and a real collector fixture before those commitments can be authority evidence. E1c then retains the exact OpenCode `user_text` workspace, listener-registration, runtime-isolation, operation-family, capability-snapshot, and snapshot-attestation chain in dependency order. Its design now forbids in-lineage workspace replacement, quarantines and never adopts pre-E1c E-purpose signer rows, stages only unsigned snapshot bytes plus a reservation at `capability_prepared`, co-lands normalized snapshot/attestation/acceptance at `accepted_inert`, and reconciles caller-retained ref-free requests through their `nbao_*`, immutable artifact links, and dense transition receipts. No second operation-token identity, migration number, or stateful implementation has landed. E1c leaves both transport pointers unchanged. I remains their sole planned non-null pair installer/replacer, and neither E1c nor I creates an admitted command, attempt, dispatch, effect, native call, production operation, or capability claim.

**Production boundary after A1.8a1-E1a:** the trusted registration/root orchestration surface is installed only when `startProductionRuntimeOwnerDaemon` receives an explicit `registrationAdapter`. The ordinary CLI passes none, so its operation registry remains empty, its authenticated RPC surface remains health-only, and health reports `ownerOperationsWritable:false` and `nativeRegistrationEnabled:false`. Wrapped Claude MITM, OpenCode, and tmux therefore remain A0 drivers with their existing process-local registration and teardown; owner failure preserves that path. A1 broker, ingress, command-adjudication, command-result-signing, and rejected-result-finalization calls remain confined to host-only dormant modules and tests: ordinary launches, drivers, runtime-owner RPC, `HostRcRelay`, and the viewer invoke none of them. Schema-v7 route receipts through schema-v11 terminal rows are dormant recovery/authority evidence, not dispatch or delivery authority. A1.7b1 plus A1.8a0 still advertise nothing; E0 and E1a are pure host-state exports with no authority operation or runtime wiring; and E1b/E1c/I do not exist. A1.8a2 must add the admitted native attempt/front-door dispatch/effect arm atomically, and A1.8b must seal and publish retained delivery intents before any live result capability exists.
...
| A1.8a1-E1b | Implement exact collector-backed Linux/OpenCode child evidence | Planned next; every registered child codec plus a real collector fixture must reproduce the four E1a parents |
| A1.8a1-E1c | Retain exact signed OpenCode `user_text` native-binding authority evidence without changing either transport pointer | Planned after E1b under frozen workspace, legacy-signer, staging, artifact-link, transition-receipt, and exact-reconciliation rules; no migration number yet |
