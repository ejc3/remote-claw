...
**Status:** selected architecture; A1.0 through dormant A1.8a0 are implemented. The ordinary production CLI supplies no native registration adapter and invokes no A1 broker, ingress actor, command adjudicator, server signer, or result finalizer, so every real driver and the viewer remain on their A0 paths. A1.7a retains ingress evidence through `awaiting_order`; schema v9 supplies server-scoped key custody/signing authority; schema v10 orders an A1-ingress command, freezes a rejected decision, and signs a replaceable result preparation; and schema v11 now atomically closes only that already-signed rejected arm into an immutable common result, dense signer acceptance, terminal adjudication overlay, exact semantic-result artifact, and one plaintext `pending_seal` delivery intent. A1.8a0 adds no admitted/effect arm, encryption, output part/signature, claim/publish path, cursor movement, viewer projection, broker call, driver operation, production wiring, or capability advertisement. Full A1.8a admitted atomic arming and A1.8b sealing/publishing remain planned.
...
| A1.8a | Add the admitted native attempt/front-door dispatch/protected effect arm to the same atomic boundary | Planned next; must not widen A1.8a0's rejected-only finalizer or advertise a partial path |
