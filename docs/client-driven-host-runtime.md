...
**Status:** selected architecture; A1.0 through dormant A1.8a0 are implemented. A1.8a1-E0 implements the six E-side canonical ID contracts and four deterministic attestation/snapshot ID derivations; A1.8a1-E1a implements the four pure, strict, 64 KiB ref-free parent-envelope codecs, closed ordered role/schema/bound/scope registry, locked byte/digest vectors, and bounded raw-artifact digest helper. A1.8a1-E1b1 implements strict role-correlated native/front-door executable-content manifest codecs plus a direct-only Linux collector that observes one regular executable through two stable passes over one descriptor. The native role has retained proof from the real pinned OpenCode 1.17.5 Linux arm64 binary. Generic collection also supports the front-door role and has temporary-file coverage, but no retained or provenance-bound actual front-door observation exists. E1b1 proves no pathname, running process, front door, currentness, complete parent, authority, or production capability. A1.8a1-E1b2 is now implemented and its gate has passed: four strict full-u64 leaf codecs close the exact mount-namespace → canonical-directory → filesystem-identity → allowed-root-ancestry digest DAG; a bounded raw-byte verifier snapshots, reparses, canonically re-encodes, and verifies the four leaf views plus separately supplied E1a workspace-parent bytes; and a direct-only Linux collector performs the frozen synchronous independent no-follow observation. The direct unprivileged `unshare -Ur -m` test exercises real mount-namespace and bind-mount behavior without silently skipping. E1b2 remains historical-only: it proves no currentness, running process, front door, authority, stateful acceptance, production operation, or capability. E1b3 front-door/listener, E1b4 isolation, and E1b5 capability/full-parent closure remain planned. A1.8a1-E1c then owns stateful accepted evidence only after E1b5, under the frozen workspace, legacy-signer, phase-staging, and exact-reconciliation rules in the [technical reference](client-driven-host-runtime-reference.md#41-a18a1-native-binding-authority-freeze-planned-dormant), without changing either transport pointer. A1.8a1-I remains planned after E1c, A1.8a2 admitted atomic arming remains planned after I, and A1.8b still owns sealing and publishing.
...
**A1.8a1 native-binding authority (E0/E1a/E1b1/E1b2 implemented; E1b3–E1b5/E1c/I implementation planned):** E0 supplies identities. E1a supplies only the four canonical parents and ref-free `role/schema/digest/byteLength` commitments; every future `rcph_*` is repository-local graph state and never part of a parent, recipe, or caller request identity. E1b1 closes the bounded executable-content manifest grammar and generic stable-FD collection for both executable roles, plus the native role's retained OpenCode fixture; it does not retain or provenance-bind an actual front door. E1b2 closes its strict full-u64 mount-namespace → canonical-directory → filesystem-identity → allowed-root-ancestry codecs, raw-five-view DAG/parent verifier, synchronous independent no-follow root/target collector, same-mount suffix rule, and non-skipping direct unprivileged namespace/bind-mount gate. The collector returns only the four canonical leaf artifacts; a later caller must separately build the context-bearing E1a parent and submit its raw bytes with the four raw leaf views to the verifier. E1b2 remains direct-only historical evidence rather than current authority. E1b3 must close the actual front-door/listener chain, E1b4 isolation, and E1b5 capability plus all four parents before any commitment can be authority evidence. E1c then retains the exact OpenCode `user_text` workspace, listener-registration, runtime-isolation, operation-family, capability-snapshot, and snapshot-attestation chain in dependency order. Its design forbids in-lineage workspace replacement, quarantines and never adopts pre-E1c E-purpose signer rows, stages only unsigned snapshot bytes plus a reservation at `capability_prepared`, co-lands normalized snapshot/attestation/acceptance at `accepted_inert`, and reconciles caller-retained ref-free requests through their `nbao_*`, immutable artifact links, and dense transition receipts. No second operation-token identity, migration number, or stateful implementation has landed. E1c leaves both transport pointers unchanged. I remains their sole planned non-null pair installer/replacer, and neither E1c nor I creates an admitted command, attempt, dispatch, effect, native call, production operation, or capability claim.

**Production boundary after A1.8a1-E1b2:** the trusted registration/root orchestration surface is installed only when `startProductionRuntimeOwnerDaemon` receives an explicit `registrationAdapter`. The ordinary CLI passes none, so its operation registry remains empty, its authenticated RPC surface remains health-only, and health reports `ownerOperationsWritable:false` and `nativeRegistrationEnabled:false`. Wrapped Claude MITM, OpenCode, and tmux therefore remain A0 drivers with their existing process-local registration and teardown; owner failure preserves that path. A1 broker, ingress, command-adjudication, command-result-signing, and rejected-result-finalization calls remain confined to host-only dormant modules and tests: ordinary launches, drivers, runtime-owner RPC, `HostRcRelay`, and the viewer invoke none of them. Schema-v7 route receipts through schema-v11 terminal rows are dormant recovery/authority evidence, not dispatch or delivery authority. A1.7b1 plus A1.8a0 still advertise nothing; E0, E1a, and E1b2's workspace codecs/verifier are host-state exports, while the E1b1 and E1b2 filesystem-reading collectors are direct-only and absent from every driver/runtime-owner/relay/viewer path. None creates an authority operation or runtime wiring, and the E1b3–E1b5/E1c/I implementations do not exist. A1.8a2 must add the admitted native attempt/front-door dispatch/effect arm atomically, and A1.8b must seal and publish retained delivery intents before any live result capability exists.
...
Milestones group dependency-ordered work; they are not promises to put an entire milestone into one pull request. Each numbered slice in the [delivery-plan reference](client-driven-host-runtime-reference.md#13-delivery-plan) lands as its own reviewed PR and leaves capabilities disabled until that slice's proof gate passes. A0.1 and A0.2 are complete process-local compatibility milestones; A1.0 through dormant A1.8a0 are implemented stateful foundations, including the closed trusted-adapter registration/root seam, dormant broker route installation, evidence-preserving ingress, server signing, rejected command adjudication, and rejected-only atomic terminal closure into an inert `pending_seal` intent. A1.8a1-E0 identities, E1a ref-free parent envelopes, E1b1 executable-content manifests/collector, and E1b2 workspace codecs/verifier/collector are implemented; E1b1 and E1b2 remain non-authoritative and direct-only. E1b3 front-door/listener, E1b4 isolation, and E1b5 capability/full-parent closure follow; E1c then owns stateful accepted evidence; I follows E1c; and A1.8a2's admitted/effect arm follows I. A1.7b1, A1.8a0, E0, E1a, E1b1, and E1b2 still advertise nothing. A1.8b, A1.9, A1.10, A1.11, A2, N1, B, C, and D then own sealing/publishing, dispatch recovery, inference, viewer integration, nested operation, and outward connectors.
...
| A1.8a1-E1b2 | Implement exact workspace evidence | Implemented and gate passed: exact four-leaf full-u64 digest DAG, bounded raw-five-view verifier against separately supplied E1a parent bytes, synchronous independent no-follow root/target walks, two sweeps and fresh rewalks, same-mount suffix policy, and non-skipping direct unprivileged namespace/bind-mount proof; historical-only, direct-only, and non-authoritative |
| A1.8a1-E1b3 | Implement actual front-door and listener evidence | Next; must collect the front-door executable and close build, generated surface, route registry, measured dispatch, and listener parent |
...
The design does not claim a capability until tests establish it. A1.2's persistence-only bootstrap, selector replacement, inventory, coordinator lease/journal reconciliation, and semantic-validation gates have landed. A1.3 proves the runtime-owner schema/repository, runtime-ID vector and key custody, service fencing and takeover, authenticated bounded RPC, daemon/bootstrap cleanup, and multi-runtime isolation. A1.4 additionally proves canonical evidence, the complete schema-v5 registration graph, exact replay/collision and reconciliation, bounded reverse callable-port transport, stale-open crash recovery, current-fenced transport reattach, and lifecycle/publication closure. Its semantic validator also implements no-extra-row closure. A1.5 proves strict pure wire/certificate/onboarding codecs and the complete schema-v6 terminal-root ledger: preparation/finalization, signature verification, activation, renewal, exact replay/collision, stale-fence and expired-preparation refusal, rollback and lost-commit reconciliation, rooted reopen validation, and lifecycle demotion followed by re-ready renewal. A1.6 proves exact capability negotiation, bearer/route/store authority, durable ciphertext and route-wide retry/collision semantics, rollover/manifests and bounded one-generation pagination, browser-safe outcome-unknown handling, schema-v7 capability-pin/route/pristine-genesis persistence, and host-only split-commit recovery. A1.7a adds real-SQLite proof for schema-v8 route backfill/auto-seeding, exact evidence closure, current-fenced actor takeover, bounded page staging, separate fetch/semantic progress, multipart assembly, exact retry, collision/incomplete tombstones, audited recovery, quota/lookahead limits, secure reopen, and completion through durable `awaiting_order`. A1.7b0 proves server-key wrapping/AAD transplant resistance, no raw key export, initial self-anchor installation, coordinator/lease fencing, immutable signer sequences and payload bindings, signed-record acceptance, exact replay/collision, stale-authority refusal, reconciliation, and schema-v9 semantic reopen. A1.7b1 proves the schema-v10 exact five-table boundary, ready/control-journal union, route/global head ordering, deterministic IDs/digests, rejected-only decision graph, distinct creation/decision fences, signer binding/signature storage, repeated abort/reprepare generations, crash reconciliation, and signed-but-unaccepted reopen. A1.8a0 proves the exact schema-v11 three-table closure, rejected action/chat payload bytes and stored-result/delivery digests, one-transaction rollback, landed/absent unknown-commit reconciliation, post-sign route collision/closure independence, narrow successor acceptance of a valid predecessor-lease signature, hostile-SQL corruption refusal, and absence from the production import graph. A1.8a1-E0 proves the six E-side canonical namespace/type/parser contracts and four deterministic ID derivations. E1a proves the four ref-free bounded parent codecs and commitment registry but no child semantics or authority. E1b1 proves the strict two-role executable manifest grammar, bounded 64 KiB artifacts over 1 MiB chunks and at most 256 MiB of source bytes, two complete same-descriptor Linux reads with stable metadata/EOF checks, role/schema transplant rejection, and an independently rebuilt retained 150-chunk OpenCode vector. That gate proves only native executable content; it does not prove a pathname, process, front door, currentness, complete parent, or authority. E1b2's gate has passed: it proves the four strict full-u64 leaf codecs and exact M→P→F→A dependencies; snapshots, reparses, re-encodes, and hashes all four raw leaf views plus separately supplied raw E1a parent bytes; rejects dependency/runtime/incarnation/path/ancestry/race/namespace/mount/cleanup failures; performs live synchronous direct-only Linux collection; and runs real namespace/bind-mount coverage under non-skipping `unshare -Ur -m`. The result remains historical evidence with no currentness, running-process, authority, stateful acceptance, or production claim. E1b3 must prove actual front-door/listener evidence, E1b4 isolation, and E1b5 capability plus complete recreation of all four E1a parents. E1c may then prove dependency-ordered accepted runtime-owner signatures, frozen workspace and legacy-signer rules, exact snapshot staging, immutable artifact links and transition receipts, request-bound reconciliation, semantic reopen, and byte-identical preservation of both transport pointers. I must then prove the remaining install identities, the null/null-or-matched pointer invariant, atomic pair replacement/withdrawal, channel and live-port fencing, parent close/reattach/takeover, and continued absence from every production operation/import path before A1.8a2 may consume that authority. Production still advertises neither native registration nor live A1 mutation because no ordinary launch installs a real adapter or invokes the dormant A1 modules. The largest remaining live/integrated proofs are:
