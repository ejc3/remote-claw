...
**A1.0 through A1.3 today:** the canonical field writer, host-state contracts, Linux-only secure SQLite kernel, migrations, synchronous high-level transactions, and verified protected artifacts have landed. A1.2 supplies schema v3 and the dormant server/project/chat/binding/coordinator repository. A1.3 supplies schema v4, the runtime-owner repository and semantic validator, exact `rcrt_*` identity, an independently supervised owner daemon, authenticated and resource-bounded local RPC, process-start-bound service leases and takeover, wrapped Ed25519 custody, and durable multi-runtime/multi-conversation ownership records. Migration 4 has 141 ordered statements and digest `zx52EtAFNY9hEZneG3RW14zRCYR18gg7ysnltHbOkT0`; the full v4 manifest is 231 rows: 30 tables, 57 indexes, and 144 triggers. Wrapped `--rc-app` MITM, OpenCode, and tmux CLI paths now connect to or best-effort start that owner after identity load, while plain launches, help, trace mode, and the local `--rc-identity` action do not. The only successful production owner operation is authenticated health; its dispatch registry is empty, and health reports `ownerOperationsWritable:false` and `nativeRegistrationEnabled:false`. Existing A0 drivers remain behaviorally unchanged, owner failure falls back to the exact A0 path, and no durable owner registration, A1 binding activation, terminal root, owner dispatch, or new A1 remote mutation, inference, or broker capability is enabled. On wrapper exit, remote-claw closes only the owner RPC connection, leaving the daemon and its service lease alive; each A0 driver still performs its existing native teardown. A1.4 is next.

**The A1.2 boundary:** each chat stores the exact selector-mapping generation it used; replacing the current terminal mapping affects later selection without silently moving old chats. Persisted v3 chats remain recovering, their bindings unresolved and starting, and their random `rcie_*` native-harness edges installing and non-writable. Evidence refs and digests remain opaque until A1.4 verifies them and owns native setup/callable ports. A1.2's actor scopes are durable addresses only; A1.7 owns ingress queues, admission, and per-chat serialization. Nested mappings and remote-server edges remain rejected until N1.
...
The runtime owner is now independently supervised and persists its own service epoch. Schema v4 and the landed repository can persist runtime inventory, wrapped keys, and local conversation transitions, but no production path creates those records or registers a native runtime yet. The native client endpoint, private provider façade, and inference connector remain later integrations. If the coordinator or its journal is unavailable at cold start, the local TUI may still use the native harness, but remote collaboration stays unavailable and non-writable until its epoch and journal recover. Coordinator-independent transition persistence applies only inside an already durable project; first-project allocation remains coordinator-owned.
...
| A1.4–A1.11 | Durable registration orchestration, wire/signing activation, broker/ingress actors, outbox/effect gates, inference, projection, and recovery | Planned; A1.4 next |
...
The design does not claim a capability until tests establish it. A1.2's persistence-only bootstrap, selector replacement, inventory, coordinator lease/journal reconciliation, and semantic-validation gates have landed. A1.3 additionally proves the runtime-owner schema/repository, runtime-ID vector and key-custody tests, service fencing and explicit-release takeover, authenticated bounded RPC, daemon/bootstrap cleanup, multi-runtime isolation, and health-only production boundary; it still advertises no durable owner registration or A1 mutation. The largest remaining live/integrated proofs are:

- A1.4 consuming A1.2's durable project/chat reservation after resolving its opaque evidence and live callable port, without `project:null`, cwd/title inference, only/most-recent fallback, or a duplicate graph after a lost response;
