...
**Status:** selected architecture; A1.0 through dormant A1.8a0 are implemented. The ordinary production CLI supplies no native registration adapter and invokes no A1 broker, ingress actor, command adjudicator, server signer, result finalizer, or native-binding authority operation, so every real driver and the viewer remain on their A0 paths. A1.7a retains ingress evidence through `awaiting_order`; schema v9 supplies server-scoped key custody/signing authority; schema v10 orders an A1-ingress command, freezes a rejected decision, and signs a replaceable result preparation; and schema v11 atomically closes only that already-signed rejected arm into an immutable common result, dense signer acceptance, terminal adjudication overlay, exact semantic-result artifact, and one plaintext `pending_seal` delivery intent. The next design is now frozen, not implemented: A1.8a1-E retains accepted-but-inert OpenCode `user_text` binding evidence without changing either transport authority pointer, and A1.8a1-I later installs one exact signed capability snapshot plus one credentialless authenticated callable-port ingress lease by atomically changing both pointers together. Neither stage creates an admitted command, attempt, dispatch, effect, native call, production wiring, or capability advertisement. A1.8a2 admitted atomic arming and A1.8b sealing/publishing remain planned.
...
**A1.8a1 native-binding authority design (frozen, unimplemented):** A1.8a1 is split at two closed boundaries. A1.8a1-E retains exact workspace, listener-registration, runtime-isolation, operation-family, capability-snapshot, and snapshot-attestation evidence in dependency-ordered signed phases; key rotation is barred while a phase is nonterminal. Accepted evidence is deliberately non-authoritative and never changes either existing transport pointer: first-install evidence leaves null/null, while replacement evidence leaves the predecessor pair intact. A1.8a1-I is the sole future installer/replacer of a non-null pair: one transaction must install or replace the accepted snapshot and a credentialless authenticated callable-port ingress lease as a matched pair. Initialization, withdrawal, and parent lifecycle may write null/null only. The ingress reuses the exact live A1.4 native-conversation callable port and stores no URL, socket path, descriptor, bearer, provider credential, or readable secret. Authority withdrawal changes only the exact durable pair; only A1.4 parent closure/replacement may unregister the shared physical port, so late predecessor cleanup cannot disable a successor. Restart must withdraw and replace lost volatile authority rather than reconstruct it from SQLite. This remains synthetic and dormant because the selected OpenCode listener/manifest/isolation producer is not implemented. The exhaustive identities, evidence joins, pair-CAS, lifecycle, unknown-commit, validator, and exclusion contract is [frozen in the technical reference](client-driven-host-runtime-reference.md#41-a18a1-native-binding-authority-freeze-planned-dormant).

**Production boundary after A1.8a0:** the trusted registration/root orchestration surface is installed only when `startProductionRuntimeOwnerDaemon` receives an explicit `registrationAdapter`. The ordinary CLI passes none, so its operation registry remains empty, its authenticated RPC surface remains health-only, and health reports `ownerOperationsWritable:false` and `nativeRegistrationEnabled:false`. Wrapped Claude MITM, OpenCode, and tmux therefore remain A0 drivers with their existing process-local registration and teardown; owner failure preserves that path. A1 broker, ingress, command-adjudication, command-result-signing, and rejected-result-finalization calls remain confined to host-only dormant modules and tests: ordinary launches, drivers, runtime-owner RPC, `HostRcRelay`, and the viewer invoke none of them. Schema-v7 route receipts through schema-v11 terminal rows are dormant recovery/authority evidence, not dispatch or delivery authority. A1.7b1 plus A1.8a0 still advertise nothing, and design-frozen A1.8a1 adds no current operation or import path. A1.8a2 must add the admitted native attempt/front-door dispatch/effect arm atomically, and A1.8b must seal and publish retained delivery intents before any live result capability exists.
...
| A1.8a1-E | Retain exact signed OpenCode `user_text` native-binding authority evidence without changing either transport pointer | Design frozen; implementation next; first-install pointers stay null/null and replacement leaves the predecessor pair intact |
| A1.8a1-I | Atomically install or replace one accepted capability snapshot plus one credentialless authenticated callable-port ingress lease | Planned after E; both transport pointers change together; still no admitted command, dispatch, effect, native call, or production operation |
