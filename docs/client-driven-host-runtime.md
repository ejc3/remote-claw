...
**Status:** selected architecture; A1.4's closed provider-neutral trusted-adapter seam is implemented, and A1.5 is next. The ordinary production CLI supplies no native registration adapter, so every real driver remains on its A0 path.
...
The runtime owner is independently supervised and persists its own service epoch. Schema v5 and the landed repositories can persist runtime inventory, wrapped keys, local conversation transitions, and exact native-registration lease/publication/operation history. The trusted A1.4 adapter seam can register and reattach that graph, but the ordinary CLI supplies no adapter, so no real production driver creates it yet. The native client endpoint, private provider façade, and inference connector remain later integrations. If the coordinator or its journal is unavailable at cold start, the local TUI may still use the native harness, but remote collaboration stays unavailable and non-writable until its epoch and journal recover. Coordinator-independent transition persistence applies only inside an already durable project; first-project allocation remains coordinator-owned.
...
| A1.5–A1.11 | Canonical wire/signing and terminal-root activation, broker/ingress actors, outbox/effect gates, inference, projection, and integrated recovery | Planned; A1.5 next |
