...
**Status:** selected architecture; A1.0 through dormant A1.8a0 are implemented, and A1.8a1-E0 now adds only the six E-side canonical ID namespaces/types/parsers (`nwb_*`, `nlra_*`, `nria_*`, `nbcs_*`, `ncsa_*`, and `nbao_*`) plus the four deterministic `nlra_*`/`nria_*`/`nbcs_*`/`ncsa_*` derivations and locked vectors. The ordinary production CLI supplies no native registration adapter and invokes no A1 broker, ingress actor, command adjudicator, server signer, result finalizer, or native-binding authority operation, so every real driver and the viewer remain on their A0 paths. E0 adds no evidence digest or artifact codec, SQLite schema or migration, repository, signer mutation, key-rotation barrier, transport-pointer write, callable-port access, owner operation, protocol packet, or production capability. The remaining accepted-evidence boundary is A1.8a1-E1, whose design is not frozen; the [technical reference](client-driven-host-runtime-reference.md#41-a18a1-native-binding-authority-freeze-planned-dormant) records its five blocking contract groups. A1.8a1-I remains planned after E1, A1.8a2 admitted atomic arming remains planned after I, and A1.8b still owns sealing and publishing.
...
**A1.8a1 native-binding authority (E0 identity contract implemented; E1/I planned):** A1.8a1-E0 is identity vocabulary only; it retains no workspace, listener, isolation, capability, signature, or authority row. A1.8a1-E1 is the remaining accepted-evidence boundary, intended to retain exact OpenCode `user_text` workspace, listener-registration, runtime-isolation, operation-family, capability-snapshot, and snapshot-attestation evidence in dependency-ordered signed phases without changing either transport pointer. E1 is blocked until five contract groups are exact: the canonical parent and platform/build/kernel child codecs beneath `remote-claw/native-workspace-binding-input/v1`, `remote-claw/native-listener-evidence-input-vector/v1`, `remote-claw/native-isolation-evidence-input-vector/v1`, and `remote-claw/native-binding-capability-input-vector/v1`; the workspace/link creation, generation, CAS, uniqueness, replay, and terminal lifecycle; schema-v12 treatment of any pre-v12 rows using the three reserved E signer purposes; the capability-snapshot/attestation staging cycle and per-phase row inventory; and every repository request/result plus retained original request used for unknown-commit reconciliation. A1.8a1-I remains the sole planned installer/replacer of a non-null pair: one transaction must install or replace an accepted snapshot and credentialless authenticated callable-port ingress lease together. First-install E1 evidence must leave null/null, replacement evidence must preserve the predecessor pair byte-identically, initialization/withdrawal/parent lifecycle may write null/null only, and neither E1 nor I may create an admitted command, attempt, dispatch, effect, native call, production operation, or capability claim. The [technical reference](client-driven-host-runtime-reference.md#41-a18a1-native-binding-authority-freeze-planned-dormant) distinguishes the implemented E0 bytes from the unresolved E1 design and planned I contract.

**Production boundary after A1.8a1-E0:** the trusted registration/root orchestration surface is installed only when `startProductionRuntimeOwnerDaemon` receives an explicit `registrationAdapter`. The ordinary CLI passes none, so its operation registry remains empty, its authenticated RPC surface remains health-only, and health reports `ownerOperationsWritable:false` and `nativeRegistrationEnabled:false`. Wrapped Claude MITM, OpenCode, and tmux therefore remain A0 drivers with their existing process-local registration and teardown; owner failure preserves that path. A1 broker, ingress, command-adjudication, command-result-signing, and rejected-result-finalization calls remain confined to host-only dormant modules and tests: ordinary launches, drivers, runtime-owner RPC, `HostRcRelay`, and the viewer invoke none of them. Schema-v7 route receipts through schema-v11 terminal rows are dormant recovery/authority evidence, not dispatch or delivery authority. A1.7b1 plus A1.8a0 still advertise nothing; E0 is a pure host-state identity export with no authority operation or runtime wiring; and E1/I do not exist. A1.8a2 must add the admitted native attempt/front-door dispatch/effect arm atomically, and A1.8b must seal and publish retained delivery intents before any live result capability exists.
...
| A1.8a1-E1 | Retain exact signed OpenCode `user_text` native-binding authority evidence without changing either transport pointer | Design not frozen; blocked on the five contract groups above; first-install pointers must stay null/null and replacement must leave the predecessor pair byte-identical |
| A1.8a1-I | Atomically install or replace one accepted capability snapshot plus one credentialless authenticated callable-port ingress lease | Planned after E1; both transport pointers change together; still no admitted command, dispatch, effect, native call, or production operation |
