...
**A1.0 through A1.2 today:** the canonical field writer, host-state contracts, Linux-only secure SQLite kernel, migrations, synchronous high-level transactions, verified protected artifacts, schema-v3 durable host records, and high-level repository have landed as dormant libraries. A1.2 can atomically bootstrap the default server/profile and first project/chat/binding/intent/installing-edge graph, create later explicit projects and chats, replace a terminal selector mapping by generation compare-and-swap, inventory the complete supported host graph needed for restart, and acquire/renew/release/reconcile coordinator leases with an immutable contiguous journal. Every existing v3 database receives full semantic validation in the same read-only WAL snapshot used before writable open; a newly migrated graph is validated before its handle returns. No run, registrar, RC, coordinator, or native-driver path imports or opens this kernel; production creates no database or A1 row, acquires no lease, performs no native effect, starts no broker handshake, and advertises no writable A1 behavior. A1.3 is next.
...
| A1.2 | Schema-v3 default server/profile, projects and terminal selector generations, recovering chats, starting bindings/intents, installing `rcie_*` terminal edges, coordinator leases, immutable journal, reconciliation/inventory, and full semantic snapshot validation | Implemented (dormant; no production opens, actor queues, native effects, or nested edges) |
| A1.3–A1.11 | Runtime owner, durable registration orchestration, wire/signing, broker/ingress actors, outbox/effect gates, inference, projection, and recovery | Planned; A1.3 next |
...
The design does not claim a capability until tests establish it. A1.2's persistence-only bootstrap, selector replacement, inventory, lease/journal reconciliation, and semantic-validation gates have landed as dormant tests; the largest remaining live/integrated proofs are:
