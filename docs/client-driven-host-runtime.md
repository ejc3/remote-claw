...
**Status:** selected architecture; A1.0 through dormant A1.8a0 are implemented. A1.8a1-E0 implements the six E-side canonical ID contracts and four deterministic attestation/snapshot ID derivations; A1.8a1-E1a implements the four pure, strict, 64 KiB ref-free parent-envelope codecs, closed ordered role/schema/bound/scope registry, locked byte/digest vectors, and bounded raw-artifact digest helper. A1.8a1-E1b1 now implements strict role-correlated native/front-door executable-content manifest codecs plus a direct-only Linux collector that observes one regular executable through two stable passes over one descriptor. The native role has retained proof from the real pinned OpenCode 1.17.5 Linux arm64 binary. Generic collection also supports the front-door role and has temporary-file coverage, but no retained or provenance-bound actual front-door observation exists. E1b1 proves no pathname, running process, front door, currentness, complete parent, authority, or production capability. A1.8a1-E1b2's exact four-leaf digest DAG, synchronous two-chain Linux observation, historical-only boundary, and executable gate are design-frozen in the [technical reference](client-driven-host-runtime-reference.md#41-a18a1-native-binding-authority-freeze-planned-dormant), but no E1b2 codec or collector exists. E1b3 front-door/listener, E1b4 isolation, and E1b5 capability/full-parent closure remain planned. A1.8a1-E1c then owns stateful accepted evidence only after E1b5, under the frozen workspace, legacy-signer, phase-staging, and exact-reconciliation rules in that reference, without changing either transport pointer. A1.8a1-I remains planned after E1c, A1.8a2 admitted atomic arming remains planned after I, and A1.8b still owns sealing and publishing.
...
**A1.8a1 native-binding authority (E0/E1a/E1b1 implemented; E1b2 design frozen; E1b2–E1b5/E1c/I implementation planned):** E0 supplies identities. E1a supplies only the four canonical parents and ref-free `role/schema/digest/byteLength` commitments; every future `rcph_*` is repository-local graph state and never part of a parent, recipe, or caller request identity. E1b1 closes the bounded executable-content manifest grammar and generic stable-FD collection for both executable roles, plus the native role's retained OpenCode fixture; it does not retain or provenance-bind an actual front door. E1b2 must implement its frozen mount-namespace → canonical-directory → filesystem-identity → allowed-root-ancestry digest DAG, independent no-follow root/target walks, and exact workspace parent; its result remains historical rather than current authority. E1b3 must close the actual front-door/listener chain, E1b4 isolation, and E1b5 capability plus all four parents before any commitment can be authority evidence. E1c then retains the exact OpenCode `user_text` workspace, listener-registration, runtime-isolation, operation-family, capability-snapshot, and snapshot-attestation chain in dependency order. Its design forbids in-lineage workspace replacement, quarantines and never adopts pre-E1c E-purpose signer rows, stages only unsigned snapshot bytes plus a reservation at `capability_prepared`, co-lands normalized snapshot/attestation/acceptance at `accepted_inert`, and reconciles caller-retained ref-free requests through their `nbao_*`, immutable artifact links, and dense transition receipts. No second operation-token identity, migration number, or stateful implementation has landed. E1c leaves both transport pointers unchanged. I remains their sole planned non-null pair installer/replacer, and neither E1c nor I creates an admitted command, attempt, dispatch, effect, native call, production operation, or capability claim.

**Production boundary after A1.8a1-E1b1:** the trusted registration/root orchestration surface is installed only when `startProductionRuntimeOwnerDaemon` receives an explicit `registrationAdapter`. The ordinary CLI passes none, so its operation registry remains empty, its authenticated RPC surface remains health-only, and health reports `ownerOperationsWritable:false` and `nativeRegistrationEnabled:false`. Wrapped Claude MITM, OpenCode, and tmux therefore remain A0 drivers with their existing process-local registration and teardown; owner failure preserves that path. A1 broker, ingress, command-adjudication, command-result-signing, and rejected-result-finalization calls remain confined to host-only dormant modules and tests: ordinary launches, drivers, runtime-owner RPC, `HostRcRelay`, and the viewer invoke none of them. Schema-v7 route receipts through schema-v11 terminal rows are dormant recovery/authority evidence, not dispatch or delivery authority. A1.7b1 plus A1.8a0 still advertise nothing; E0 and E1a are pure host-state exports, while E1b1's filesystem-reading collector is direct-only and absent from every driver/runtime-owner/relay/viewer path. None creates an authority operation or runtime wiring, and the E1b2–E1b5/E1c/I implementations do not exist. A1.8a2 must add the admitted native attempt/front-door dispatch/effect arm atomically, and A1.8b must seal and publish retained delivery intents before any live result capability exists.
...
Milestones group dependency-ordered work; they are not promises to put an entire milestone into one pull request. Each numbered slice in the [delivery-plan reference](client-driven-host-runtime-reference.md#13-delivery-plan) lands as its own reviewed PR and leaves capabilities disabled until that slice's proof gate passes. A0.1 and A0.2 are complete process-local compatibility milestones; A1.0 through dormant A1.8a0 are implemented stateful foundations, including the closed trusted-adapter registration/root seam, dormant broker route installation, evidence-preserving ingress, server signing, rejected command adjudication, and rejected-only atomic terminal closure into an inert `pending_seal` intent. A1.8a1-E0 identities, E1a ref-free parent envelopes, and E1b1 executable-content manifests/collector are implemented; E1b1 remains non-authoritative and direct-only. E1b2 workspace, E1b3 front-door/listener, E1b4 isolation, and E1b5 capability/full-parent closure follow; E1c then owns stateful accepted evidence; I follows E1c; and A1.8a2's admitted/effect arm follows I. A1.7b1, A1.8a0, E0, E1a, and E1b1 still advertise nothing. A1.8b, A1.9, A1.10, A1.11, A2, N1, B, C, and D then own sealing/publishing, dispatch recovery, inference, viewer integration, nested operation, and outward connectors.
...
| A1.8a1-E1b2 | Implement exact workspace evidence | Design frozen; implementation next. Exact four-leaf u64-safe digest DAG, synchronous independent no-follow root/target walks, same-mount suffix policy, historical-only boundary, and workspace-parent gate live in the technical reference |
| A1.8a1-E1b3 | Implement actual front-door and listener evidence | Planned after E1b2; must collect the front-door executable and close build, generated surface, route registry, measured dispatch, and listener parent |
