...
**Status:** selected architecture; A1.0 through the dormant A1.7b1 command-adjudication foundation are implemented. The ordinary production CLI supplies no native registration adapter and invokes no A1 broker, ingress actor, command adjudicator, or server signer, so every real driver and the viewer remain on their A0 paths. A1.7a still owns durable ingress through `awaiting_order`; schema v9 supplies server-scoped key custody/signing authority; and schema v10 now materializes only A1-ingress common commands, allocates their shared ready-journal and command order, freezes a rejected decision, and durably signs a replaceable result preparation. A1.7b1 deliberately stops at a signed-but-unaccepted preparation: it creates no final common result, signer acceptance, ingress terminal state, source result/delivery outbox, native attempt/effect, viewer projection, or production wiring. A1.7b1 and the next A1.8a atomic finalization/effect tranche form one advertised capability gate, so neither may independently enable live A1 mutation.
...
**Production boundary after A1.7b1:** the trusted registration/root orchestration surface is installed only when `startProductionRuntimeOwnerDaemon` receives an explicit `registrationAdapter`. The ordinary CLI passes none, so its operation registry remains empty, its authenticated RPC surface remains health-only, and health reports `ownerOperationsWritable:false` and `nativeRegistrationEnabled:false`. Wrapped Claude MITM, OpenCode, and tmux therefore remain A0 drivers with their existing process-local registration and teardown; owner failure preserves that path. A1 broker, ingress, command-adjudication, and command-result-signing calls remain confined to host-only dormant modules and tests: ordinary launches, drivers, runtime-owner RPC, `HostRcRelay`, and the viewer invoke none of them. Schema-v7 route receipts, schema-v8 ingress rows, schema-v9 signer rows, and schema-v10 command/preparation rows are dormant recovery/authority prerequisites, not dispatch authority. A1.8a must atomically create the immutable final common result, signer acceptance, ingress terminalization, and source result/delivery outbox; an admitted arm must create its pinned native attempt/front-door dispatch/effect gate in that same transaction. Splitting any of those writes, or advertising A1.7b1 before that proof passes, is forbidden.
...
Milestones group dependency-ordered work; they are not promises to put an entire milestone into one pull request. Each numbered slice in the [delivery-plan reference](client-driven-host-runtime-reference.md#13-delivery-plan) lands as its own reviewed PR and leaves capabilities disabled until that slice's proof gate passes. A0.1 and A0.2 are complete process-local compatibility milestones; A1.0 through dormant A1.7b1 are implemented foundations, including the closed trusted-adapter registration/root seam, dormant broker route installation, evidence-preserving ingress through `awaiting_order`, the server signer, and rejected command adjudication through a signed-but-unaccepted result preparation. A1.8a is the next atomic persistence/effect tranche; A1.7b1 plus A1.8a are one advertised capability gate. A1.8b, A1.9, A1.10, A1.11, A2, N1, B, C, and D then own dispatch recovery, inference, viewer integration, nested operation, and outward connectors.
...
| A1.8a | Final common result + signer acceptance + ingress terminal + source result/delivery outbox; admitted attempt/dispatch/effect arm in the same transaction | Planned next; atomic with no split capability claim and jointly gated with A1.7b1 |
| A1.8b | One-time dispatch, protected authorization consumption, uncertainty quarantine, native read-back, and evidence-only reconciliation | Planned after the A1.7b1+A1.8a capability gate |
