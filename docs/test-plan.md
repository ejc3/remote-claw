...
| **Exploratory** | headless browser (Playwright), real `claude` runs | the polished UI in a real browser; tool-use + sub-agent traffic flowing through the relay | real |

CI keeps the unit + integration layers fast and network-free; the real-claude proofs are env-gated so
they never gate CI but are run on demand (and were run for this plan). Each PR (#24–#39) passed
`biome` + `tsc` + `vitest` + `next build` before merge.
...
23. Web client in a real browser — 🔬 Playwright screenshots
...
(cd packages/clawsec && pnpm test:run)   # 109
(cd packages/cli && pnpm test:run)       # 164
(cd apps/web && pnpm test:run)           # 11 (+2 proofs skipped)
