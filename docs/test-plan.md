...
queued text. Relay and parser tests own pending versus neutral resolved, reload, and late-pending
non-resurrection; the existing transcript browser sentinel owns the shared card behavior.

The native probe and actual phone/desktop acceptance passed on 2026-09-08: phone Allow executed once,
desktop Deny cancelled without execution, and a native-TUI winner made a held stale phone Allow inert.
Both browsers resolved neutrally without duplicate/reactivated cards after reload; all eight
