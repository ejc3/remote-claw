...
| Codex beyond accepted tuples | Current-version/read-only activity/interrupt/image acceptance is tracked in the release roadmap. Managed-Unix/legacy recovery, further versions, platforms, other controls, general files, and any per-device unsubscribe claim need their own bounded outcome without reopening the historical 0.151.0 text/status/coexistence and explicit-WS/paginated recovery results |
