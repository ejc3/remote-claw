...
| A1.8a | Planned next; one advertised gate with A1.7b1 | One transaction must create the immutable final common result, signer acceptance, ingress terminal state, and source result/delivery outbox and, for an admitted arm, its pinned native attempt/front-door dispatch/effect gate; every split or partial state rolls back and neither tranche advertises capability alone |
| A1.8b | Planned after the joint A1.7b1+A1.8a gate | One-time dispatch, protected authorization consumption, uncertainty quarantine, native read-back, and evidence-only recovery without replaying a possibly started effect |
...
preparation `signed`, group `result_signed`, and reservation `signed`. A1.8a must add an all-or-nothing
crash matrix for final result, signer acceptance, ingress terminalization, source result/delivery
outbox, and every admitted attempt/front-door-dispatch/effect arm before the joint capability can be
advertised.
