...
The mandatory proof runs without credentials or external routing. Capable hosts use direct unprivileged user/network namespaces; hosted CI may perform only fixed privileged namespace setup, followed by asserted nonzero runner IDs, empty groups, zero capability sets, and `no_new_privs` before package code. Neither path skips or runs project code privileged. After one combined focused gate, run one frozen-tree `pnpm check`, `pnpm typecheck`, and `pnpm test`, Markdown render/cross-doc sync, bounded independent review, and CI. Any code/schema/fixture change invalidates those results. A reachable production listener/state/signature/pointer/capability path, credential use, or external-network use is P0; arbitrary executable provenance, incomplete closure/surface binding, hidden/ambiguous dispatch, digest-only equality, transplant acceptance, mutable raw views, allocation overflow, nondeterministic build, repeatable resource leak, a skipped gate, or privileged project-code execution is P1.
