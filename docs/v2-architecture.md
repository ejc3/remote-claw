...
> (auto-created on first run; printed by `remote-claw --rc-identity`), so "host" ≈
> "machine"; **but you can override the secret per instance** (`remote-claw
> --rc-secret …`), so one machine can carry **several** identities (and, conversely,
> one secret could span machines). A *claude instance* = a *session* = a *space* =
> **one chat** (a single running `claude` with remote control enabled). So **2
> secrets × 10 instances = 20 spaces**. The hierarchy is **identity → its spaces
> (chats)**; a space does *not* contain sub-sessions — it *is* one chat. The web
> lists spaces (instances) across all pasted secrets, grouped by identity. (`host_id`
> below = a secret's identity, **not** a hardware id.)
...
2. **Get your secret (one-time):** run `remote-claw --rc-identity` → it ensures the
   default machine identity exists and prints the `rc1_…` secret to copy. (You can
   make more identities; pick one per run with `--rc-secret`/`--rc-id`.)
...
- Identities are named too (default = hostname, under `K_host_meta`); chats group
  under them. One identity ≈ one machine by default, but it's the secret that groups.
...
```
  ...
  └──────────────────────────────────────────┘         └─────────────────────────────────┘        └────────────────────┘
    one secret = one identity (host_id/auth); each claude instance under it = one "space"/chat
...
**Wrapper-only flags (`--rc-*`):**
- **`--rc-identity`** — *the identity command.* Ensures the default machine identity
  exists (generates a **root secret `S`**, 32 B CSPRNG, stored `0600` in the state
  dir, derives `host_id`/`auth_token`/`content_root`/keys — §4), **prints the
  pasteable `rc1_…` secret**, and **exits without launching claude**. Re-running
  prints `host_id` + status (never reprints `S` unless `--rc-show-secret`). You can
  create more than one identity (`--rc-id NAME --rc-identity`) and select among them
  per run below — so one machine can present several "accounts."
- **`--rc-secret rc1_…`** / **`--rc-id NAME`** — run **this** instance under a
  specific identity instead of the default (override the boundary per run).
...
- Relay logic (Node/TS, reimplementing the Phase-0 interception knowledge): on each
  ...
  `POST /api/relay`; subscribe (SSE) to the inbound channel, **dedup by `msg_id`**,
...
- Maintains **in-memory** relay state (catch-up log, `msg_id` seen-set, registered
  sessions, crypto state) — **recoverable from the claude session** by re-enabling
  `/remote-control` (the worker re-backfills), so **no durable store is required**;
  both wrapper and CLI are stateless (claude's on-disk session is the durable layer).
  Answers encrypted `catch_up{since=seq}` from its log, falling back to worker
  backfill (§6).
- **Heartbeats with a TTL:** once RC is enabled, the wrapper `POST /api/heartbeat
  {host_id, wrapper_instance_id, session_ids[], ts}` every ≈15 s so the broker/web
  can show which sessions are online (§3.2). Stops when the wrapper/CLI exits (the
  wrapper does **not** outlive the CLI) → the session goes offline after the TTL.
...
- **Ingest Functions** (Node runtime, fast + die): `POST /api/relay` (a ciphertext
  frame in either direction), `POST /api/hosts`, `POST /api/sessions`, all gated by
  `T_app` **and** bearer-authed by `auth_token`, all accepting **ciphertext only**.
- **Nothing is registered until RC is enabled.** A host/session row and presence
  exist **only** for sessions the operator has `/remote-control`-ed. A machine
  running `remote-claw` (as claude) with RC off is **invisible** to the broker — no
  host row, no heartbeat. (`GET /api/hosts` for a never-shared identity returns
  empty; the web still shows the identity you pasted, as empty/offline — §3.3.)
...
- **Presence / heartbeat (TTL):** while RC is enabled the wrapper `POST
  /api/heartbeat {host_id, wrapper_instance_id, session_ids[], ts}` every
  `HEARTBEAT_INTERVAL` (≈15 s). **Presence is keyed by `session_id`**, not by host:
  the broker stamps `last_seen` on each `session_id` in the beat. This is required
  because **several wrappers can run under one identity** (one `host_id`) — each has
  a distinct ephemeral `wrapper_instance_id` and reports only its own sessions, so
  the broker merges by session rather than letting one wrapper's beat clobber
  another's. **A session is online ≡ `now − last_seen < HEARTBEAT_TTL`** (≈45 s = 3
  missed beats); an **identity is online ≡ any of its sessions is online** (derived,
  computed at read time) — so the web shows per-chat dots without any push. The
  heartbeat carries only routing metadata (ids + ts), no content → zero-knowledge
  holds. (Distinct from the worker→relay keep-alive of the RC protocol; this is
  host→broker presence.)
...
host_id       = HKDF-Expand(PRK, "remote-claw/v1/host-id",       16B)  → PUBLIC routing/identity id (groups this secret's spaces; NOT itself a space)
...
Rotation = generate a **new `S`** ⇒ new `host_id` ⇒ a **new identity** (a fresh,
empty set of spaces); the old identity and **all** its spaces are dead. Delete the
old. No per-device revocation without rotating (single-secret tradeoff). `info` labels
...
`POST /api/relay {host_id, session_id, dir:"out", record_kind, seq, salt, nonce,
...
- **The cloud = relay + short live buffer only (Vercel-native, no third party).**
  ...
  **reconnects and resumes by `seq`** — any gap is refilled by the wrapper's
  ...
  polling of the same buffer (`GET /api/stream?since=seq`, non-streaming). Stream
  retention (1–7 d) is irrelevant — it's an in-flight buffer, not the record.
...
| `heartbeat` | wrapper→broker | presence `{host_id, wrapper_instance_id, session_ids[], ts}` (TTL, §3.2; keyed by `session_id`) |
| `host_register` / `session_register` / `session_update` | wrapper→broker | latest-wins **encrypted** name/title/cwd/status snapshots (`K_host_meta` / `K_session_meta`) |
...
Hierarchy: **identity (host_id) → its spaces (each space = one claude instance =
...
- **Add an identity:** paste another `rc1_…`; the checksum validates it, keys
  derive, `GET /api/sessions?host=host_id` lists that identity's spaces (instances).
- Spaces are listed gchat-style (encrypted title + last-activity, online dot),
  grouped under their identity. Routing metadata (`host_id`, `session_id`,
...
- **control** (AEAD under `control_key`, `dir:in`, `msg_id` + `expiry`,
  replay-checked): `catch_up` · `permission` · `interrupt` · `set_mode` ·
  `set_model` · `command` · `end`.
- **meta**: `accepted` (`dir:out`); `heartbeat` / `host_register` /
  `session_register` / `session_update` (wrapper→broker; names/titles encrypted
  under `K_host_meta` / `K_session_meta`).
...
Endpoints (all gated by the app key `X-RC-App-Key` §4.5, all bearer `auth_token`,
all ciphertext): `POST /api/hosts`, `GET /api/hosts`, `POST /api/sessions`,
`GET /api/sessions`, `POST /api/relay`, `POST /api/heartbeat`,
...
1. **Durable store / history → W′ (TUI is the brain).** No cloud history. The
   wrapper holds an **in-memory** per-session log on the TUI host (rebuilt from
   claude on reconnect) and serves catch-up via message-passing; claude's on-disk
   session (`.jsonl`) is the durable record and the Claude worker backfill is the
   deep-history source on RC connect. Cloud is a dumb ciphertext pipe + short live
   buffer. Optional Upstash Redis ciphertext log can be added later for offline
   browsing.
...
7. **No registration/presence until RC is enabled.** Running `remote-claw` like
   `claude` sends nothing to the broker; host registration is lazy (first
   `/remote-control`); heartbeats run only while RC is on (§3.2, §15 #2).
...
- **P1 — Crypto core.** `packages/clawsec` (TypeScript): secret gen/parse/checksum,
  ...
  browser unchanged.
- **P2 — CLI: arg-passthrough + `--rc-identity`.** `packages/cli` in Node/TS: the
  transparent wrapper skeleton (parse/strip the `--rc-*` namespace, forward all
  other args to `claude`, `--` escape), and `--rc-identity` (generate/store `S`,
  derive ids, print `rc1_…`; host registration deferred to first RC). Local only
  (mock app).
- **P3 — Vercel app skeleton.** `apps/web` (Next.js): **app-key middleware
  (`X-RC-App-Key` = `T_app`, a Vercel secret)** in front of ingest/read API routes +
  per-identity auth (`sha256(auth_token)`), per-session workflow with hook + durable
  stream, SSE endpoint. Deploy; curl round-trip with hand-rolled ciphertext.
...
1. **Fresh machine bootstrap.** `remote-claw --rc-identity` → generate root `S`
   (0600), derive `host_id`/`auth_token`/`content_root`/`control_key`/meta-keys,
...
4. **Enable RC mid-session via `/remote-control`.** Wrapper points the inner claude
   ...
   (`POST /api/hosts`) then `POST /api/sessions {enc(title), enc(cwd)}`; heartbeats
...
6. **Client first connection.** Open web app, paste `rc1_…` (or `#fragment`) →
   derive keys in-browser → `GET /api/hosts` (with the app key, §4.5) → the identity
...
8. **List an identity's spaces.** `GET /api/sessions?host=host_id` → decrypt
...
14. **Add an identity.** Paste another `rc1_…` → new `host_id` → its spaces appear.
...
→ new `host_id` = a new identity with a fresh, empty set of spaces; the old identity
...
6. W: **lazily registers** `POST /api/hosts {host_id, sha256(auth_token), enc(name=hostname)}` (first time only); log+encrypt each backfilled frame; W→V `POST /api/sessions {host_id, sid, enc(title), enc(cwd), status:active}`; `POST /api/heartbeat {host_id, wrapper_instance_id, session_ids:[sid]}` → session online
...
2. C→V `GET /api/hosts` → `[{host_id, enc(name), online}]` (empty if never shared); C decrypts name, renders the identity (its spaces via #8)

**7. Client second connection, 5 identities**
1. C has 5 secrets (localStorage) → 5 `(host_id, auth_token)`
2. for each: C→V `GET /api/hosts` (that host's bearer) → its record + `online` (any session live)
...
**14. Add an identity**
1. user pastes `rc2_…` → C derives `host_id₂` → C→V `GET /api/hosts` (bearer₂) → its spaces appear

**15. Rename identity/space**
1. C: `enc(new_name)` under `K_host_meta` (identity) or `K_session_meta` (space) → C→V `POST /api/hosts|sessions {…, enc(name|title)}` (update)
2. other devices: `GET /api/hosts|sessions` → decrypt the new name
...
| # | Scenario | Primitives |
| --- | --- | --- |
| 1 | Fresh machine bootstrap (`--rc-identity`) | `CSPRNG(S)`, `HKDF`→{host_id,auth_token,content_root,control_key,K_*_meta}, `checksum`, print `rc1_…` *(local only; no broker call — register is lazy)* |
...
| 4 | Enable `/remote-control` | `intercept`, `bridge`, `worker-SSE`, `initialize`, `backfill`, `log`, `GCM`, `lazy-register`(`/api/hosts`), `/api/sessions`, `app-key`+`bearer` |
| 5 | Launch with RC on (`--rc-share`) | `intercept`, `bridge`, `worker-SSE`, `initialize`, `log`, `lazy-register` |
| 6 | Client first connection | `HKDF`, `app-key`, `bearer`, `/api/hosts`, `GCM-open(name)`, `localStorage` |
...
| 14 | Add an identity | `HKDF(S₂)`, `app-key`, `bearer₂`, `/api/hosts` |
