...
> and read-only tool activity while ordinary Anthropic Remote Control stays active. A bounded packed-install run kept
...
outcome stops only the companion; Interrupt is never automatically retried, including after a 401.
This is not an idle-state or process-kill guarantee. Current Interrupt acceptance is tracked separately
from historical M1 in the [release roadmap](docs/release-finish-line.md).
...
a tool action, as the viewer discloses. Secret, optionless, nonblocking, and malformed forms stay in
the native UI. See [form bounds and semantics](docs/protocol.md#codex-native-choice-forms) and
[question acceptance status](docs/release-finish-line.md#codex-native-questions).
